...
const textOnImage = async (year,branch,message,igHandle) => {
    ...
    const image = await Jimp.read("bg-image/bgimages.png");
    ...
    if(message.length < 100)
    {
        ...
        fontMessage = await Jimp.loadFont(Jimp.FONT_SANS_64_WHITE);
    }
    else
    {
        console.log(message.length)
        fontMessage = await Jimp.loadFont(Jimp.FONT_SANS_32_WHITE);
    }

    const fontYearAndBranch = await Jimp.loadFont(Jimp.FONT_SANS_32_WHITE);
